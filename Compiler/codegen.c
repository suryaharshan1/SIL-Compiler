		...
		case TYPE_INT:
			switch(tree->NODETYPE){
				...
				case NODETYPE_MAIN:
					fprintf(fp,"START\n");
					fprintf(fp,"MOV SP 1000\n");
					fprintf(fp,"MOV BP 1000\n");
					fprintf(fp,"PUSH BP\n");
					fprintf(fp,"MOV BP SP\n");
					i = get_register();
				    Ltemp = Lhead;
    				while(Ltemp != NULL){
    					if(Ltemp->type != ARG_INT && Ltemp->type != ARG_BOOLEAN && Ltemp->type != ARG_REF_INT && Ltemp->type != ARG_REF_BOOLEAN && Ltemp->type != ARG_PAIR && Ltemp->type != ARG_REF_PAIR)
        					if(Ltemp->type != STYPE_PAIR){
					...
					fprintf(fp,"HALT\n");
					break;
         		case NODETYPE_PAIR_COMP:
	   					i = get_register();
		            	if(tree->Ptr1->Lentry == NULL){
		        			fprintf(fp,"MOV R%d [%d]\n",i,getBinding(tree) + tree->Ptr2->VALUE - 1);
		        		}
		        		else if(tree->Ptr1->Lentry != NULL){
		        			if(tree->Ptr1->Lentry->type == STYPE_PAIR){
			        			fprintf(fp, "MOV R%d %d\n",i,getBinding(tree) + tree->Ptr2->VALUE - 1);
			    				j = get_register();
			    				fprintf(fp, "MOV R%d BP\n",j);
			    				fprintf(fp, "ADD R%d R%d\n",i,j);
				    		...
				    		if(tree->Ptr1->Lentry->type == ARG_REF_PAIR){
				    			fprintf(fp,"MOV R%d [R%d]\n",i,i);
				    		}	
		        		}
		        		else {
		        			if(tree->Ptr2->VALUE == 1){
		        				j = gen_code_eval(tree->Ptr1->Ptr1);
		        			}
		        			else if(tree->Ptr2->VALUE == 2){
		        				j = gen_code_eval(tree->Ptr1->Ptr2);
		        			}
		        			free_register();
		        			i = j;
		        		}
			...
			switch(tree->NODETYPE){
				case NODETYPE_ID:
		        		...
		        		if(tree->Lentry == NULL){
		        			...
		        			if(tree->Lentry->type == STYPE_PAIR){
		        				fprintf(fp, "ADD R%d R%d\n",k,j);
		        			}
		        			else{
		        				fprintf(fp, "SUB R%d R%d\n",k,j);	
		        			...
		        			fprintf(fp, "MOV R%d [R%d]\n",k,k);
		        			free_register();
		        			if(tree->Lentry->type == ARG_REF_PAIR){
			    				fprintf(fp,"MOV R%d [R%d]\n",i,i);
			    				fprintf(fp,"MOV R%d [R%d]\n",k,k);
			        		}	
		        		}
	         	...
	         	case NODETYPE_FUNCTION:
	   			...
	   			case NODETYPE_PAIR_COMP:
						...
						return i;						
	   					break;
	   			default:
   					printf("Error : Unknown NODETYPE %d under TYPE_PAIR\n",tree->NODETYPE);
   				 	exit(1);
   					break;
   			}
			break;
				...
				case NODETYPE_ARR_ASGN:
   				...
   				case NODETYPE_PAIR_ASGN:
   					...
   					if(tree->Ptr1->Lentry == NULL){
	   					...
	   					free_register();
	   					free_register();
	   					free_register(); //One register extra because of pair
   					}
   					else {
   						i = get_register();
	   					fprintf(fp, "MOV R%d %d\n",i,getBinding(tree));
		...
		if(Argtemp->passType == PTYPE_VAL){
			...
			if(temp->Ptr2->Lentry != NULL){
				...
				free_register();
				 ...
				 if(temp->Ptr2->NODETYPE == NODETYPE_ARR_ID){
					i = get_register();
					Gtemp = temp->Ptr2->Ptr1->Gentry;
					j = gen_code_eval(temp->Ptr2->Ptr2);
					fprintf(fp,"MOV R%d %d\n",i,Gtemp->binding);
					fprintf(fp,"ADD R%d R%d\n",i,j);
					fprintf(fp,"PUSH R%d\n",i);
					free_register();
				...
				else if(temp->Ptr2->NODETYPE == NODETYPE_ID)
				{
					i = get_register();
