        ...
        case NODETYPE_FUNCTION : 
				...
				if(temp != NULL){
                	while(Argtemp != NULL && temp != NULL){
                	   if(!((temp->Ptr2->TYPE == TYPE_INT && Argtemp->type == STYPE_INT) ||(temp->Ptr2->TYPE == TYPE_BOOLEAN && Argtemp->type == STYPE_BOOLEAN))){
                	        printf("unmatching types passed for the function %s",NAME);yyerror("\n");exit(0);
                	   }
                     if(temp->Ptr2->NODETYPE != NODETYPE_ID && Argtemp->passType == PTYPE_REF){
...
int getBinding(struct Tnode *tree){
  if(tree->NODETYPE == NODETYPE_ARR_ID || tree->NODETYPE == NODETYPE_ARR_ASGN || tree->NODETYPE == NODETYPE_ARR_READ){
    ...
    else if(tree->NODETYPE == NODETYPE_ASGN || tree->NODETYPE == NODETYPE_READ){
